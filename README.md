...
- **Refresh Token**: `POST /users/login/refresh/`
- **Verify Token**: `POST /users/login/verify/`

#### Example:
```bash
curl -X POST http://localhost:8000/users/login/ -d '{"username": "user", "password": "pass"}'
```

### Users
#### Endpoints:
- **Create User**: `POST /users/sign-up/`
- **Retrieve User**: `GET /users/<id>/`
- **Self User Operations**: `GET, PUT, DELETE /users/self/`

#### Example:
```bash
curl -X POST http://localhost:8000/users/sign-up/ -d '{"username": "user", "password": "pass", "email": "user@example.com"}'
```

### Tenants
#### Endpoints:
- **Create Tenant**: `POST /tenants/`
- **Retrieve Tenant**: `GET /tenants/<id>/`
- **Update Tenant**: `PUT /tenants/<id>/`
- **Delete Tenant**: `DELETE /tenants/<id>/`

#### Example:
```bash
curl -X POST http://localhost:8000/tenants/ -d '{"name": "Tenant Name", "contact_email": "tenant@example.com"}'
```
...
#### Example:
```bash
curl -X POST http://localhost:8000/projects/ -d '{"name": "Project Name", "description": "Project Description"}'
```

### Billings
#### Endpoints:
- **Create Subscription**: `POST /billings/subscriptions/`
- **Retrieve Subscription**: `GET /billings/subscriptions/<id>/`
- **Create Invoice**: `POST /billings/invoices/`
- **Retrieve Invoice**: `GET /billings/invoices/<id>/`

#### Example:
```bash
curl -X POST http://localhost:8000/billings/subscriptions/ -d '{"tenant": "tenant_id", "subscription_tier": "Pro"}'
```

### Resources
#### Endpoints:
- **Upload Media**: `POST /resources/media/`
- **Delete Media**: `DELETE /resources/media/<id>/`
- **Handle CSV**: `POST /resources/csv/`

#### Example:
```bash
curl -X POST http://localhost:8000/resources/media/ -F 'file=@path/to/file.jpg' -F 'project_id=project_id'
```
